...
import { stores as fallbackStores } from '@/data/content';
...
export default function StoreLocations() {
  ...
  return (
    <section
      id="stores"
      className="relative overflow-hidden bg-gradient-to-b from-cream-100 to-beige-100 py-24 lg:py-32"
    >
        ...
        <div className="mt-16 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {loading
            ? [0, 1, 2, 3].map((i) => (
                <div
                  key={i}
                  className="overflow-hidden rounded-4xl bg-cream-50 shadow-lg shadow-honey-400/5"
                >
                  <div className="aspect-[3/2] animate-pulse bg-beige-100" />
                  <div className="p-5">
                    <div className="h-5 w-2/3 animate-pulse rounded bg-beige-100" />
                    <div className="mt-2 h-4 w-full animate-pulse rounded bg-beige-100" />
                    <div className="mt-4 h-4 w-24 animate-pulse rounded-full bg-beige-100" />
                  </div>
                </div>
              ))
            : displayStores.map((store, idx) => (
                <article
                  key={store.id}
                  className={`reveal group overflow-hidden rounded-4xl bg-cream-50 shadow-lg shadow-honey-400/5 transition-all duration-500 hover:shadow-2xl hover:shadow-honey-400/15 hover:-translate-y-2 ${
